...
CREATE TABLE `v_location_nearby` (
  ...
  PRIMARY KEY (`uid`),
  UNIQUE KEY `unq_location_uid` (`uid`),
  KEY `idx_locationnearby_locprivate` (`loc_private`),
  KEY `idx_locationnearby_sex` (`sex`),
...
/*Table structure for table `v_lvl` */
...
CREATE TABLE `v_order` (
  ...
  KEY `idx_order_paytype` (`pay_type`),
...
CREATE TABLE `v_paylog` (
  ...
  PRIMARY KEY (`id`),
  KEY `uid` (`uid`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='此表只是用来记录支付通知流水，不做他用';

/*Table structure for table `v_trace` */

CREATE TABLE `v_trace` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `uid` varchar(15) NOT NULL COMMENT 'uid',
  `points` text NOT NULL COMMENT '足迹点的json串',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `unq_trace_hash` (`hash`),
  KEY `idx_trace_uid` (`uid`),
...
CREATE TABLE `v_user` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `pwd` varchar(512) DEFAULT NULL COMMENT '密码',
  `name` varchar(15) DEFAULT NULL COMMENT '姓名',
  `nick` varchar(50) DEFAULT NULL COMMENT '昵称',
  `email` varchar(255) DEFAULT NULL COMMENT 'email',
  `phone` varchar(15) DEFAULT NULL COMMENT '电话',
  `status` varchar(10) NOT NULL COMMENT '状态',
  `lvl` char(5) DEFAULT NULL COMMENT '会员级别',
  ...
  `avatar_uri` varchar(60) DEFAULT NULL COMMENT '头像',
  `photo_uri` varchar(550) DEFAULT NULL COMMENT '相片',
